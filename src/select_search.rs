...
use vertigo::{bind, css, dom, DomNode, Value, dom_element, Computed, computed_tuple, transaction, KeyDownEvent};
    ...
    V: Clone + ToString + PartialEq + 'static,
{
    pub fn mount(self) -> DomNode {
        let Self { value, options, params } = self;
        ...
        let items = computed_tuple!(options, filter)
            .map(move |(inner_options, inner_filter)| {
                if let Some(inner_filter) = inner_filter {
                    let inner_filter = inner_filter.to_lowercase();
                    if inner_filter.len() >= params.min_chars {
                        // Filter options
                        inner_options.into_iter()
                            .filter(|(_, opt_value)| opt_value.to_string().to_lowercase().contains(&inner_filter))
                            .collect::<Vec<_>>()
                    } else {
                        vec![]
                    }
        ...
        let list = bind!(
            value, filter, dropdown_opened,
            list_deps.render_value(
                move |(inner_dropdown_opened, inner_items, item_selected)| {
        ...
        let input = input_deps.render_value(move |(inner_value, options_inner)| {
            ...
            let displayed_value = filter.to_computed().map(move |inner_filter|
                if let Some (inner_filter) = inner_filter {
                    ...
                    options_inner.get(&inner_value).map(|val| val.to_string()).unwrap_or_default()
                }
            );
            ...
            let hook_key_down = bind!(value, item_selected, filter, options, items, dropdown_opened, |key_down: KeyDownEvent| {
                    ...
                    transaction(|ctx| {
                        if filter.get(ctx).is_some() {
                            ...
                            if let Some(inner_item_selected) = item_selected.get(ctx) {
                                ...
                                if let Some((next_key, _)) = items_iter.skip_while(|(opt_key, _)| opt_key != &inner_item_selected).nth(1) {
