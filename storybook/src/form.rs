...
impl From<&MyModel> for FormData {
    fn from(value: &MyModel) -> Self {
        Self {
            sections: vec![
                DataSection::with_string_field("Slug", "slug", &value.slug),
                DataSection::with_string_field("Name", "name", &value.name),
    ...
    fn from(value: &MySecondModel) -> Self {
        ...
        Self {
            sections: vec![
                ...
                DataSection::new("Gender")
                    .add_list_field("gender", value.gender.to_string(), gender_map)
...
impl From<FormExport> for MySecondModel {
    fn from(form_export: FormExport) -> Self {
        Self {
            ...
            photo: form_export.file("photo").map(|dfi| dfi.name).unwrap_or_default(), // TODO
        }
    }
}
...
pub fn Form2() {
    ...
    dom! {
        <div>
            ...
            {form}
            <h4>"Model 2:"</h4>
            <p>{my_second_model.map(|m| m.first_name)} " / " {my_second_model.map(|m| m.surname)}</p>
            // <p>{my_second_model.map(|m| m.dimension_x)} "x" {my_second_model.map(|m| m.dimension_y)}</p>
        </div>
    }
}
